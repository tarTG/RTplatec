    ...
    _steps(0)
{
    ...
    std::for_each(hmap.raw_data(), hmap.raw_data() +A, [&](auto& value) {value = (value - lowest) / (highest - lowest);});
    ...
    while (th_step > 0.01)
    {
        uint32_t count = std::count_if(hmap.raw_data(), hmap.raw_data() +A, [&](auto& value){return value < sea_threshold;});
    ...
    std::for_each(hmap.raw_data(), hmap.raw_data() +A, 
...
void lithosphere::createPlates()
{
    try {
        ...
        uint32_t n= 0;
        std::generate(imap.raw_data(), imap.raw_data() + map_area,[&]{return n++;});
        ...
        if(std::any_of(imap.raw_data(),imap.raw_data() + map_area,[&](auto& val)
            {
                return val > num_plates;
            }
            ))
            {
                ASSERT(false, "A point was not assigned to any plate");
            }
        ...
        for (uint32_t i = 0; i < num_plates; ++i) {
            ...
            {
                uint32_t y_mod = _worldDimension.yMod(y);
                for (uint32_t x = x0; x < x1; ++x, ++tmp_pmap) 
                {
                    
                    uint32_t k = _worldDimension.indexOf(_worldDimension.xMod(x), y_mod);
                    *tmp_pmap = hmap[k] * (imap[k] == i);
                }
            }
            // Create plate.
...
void lithosphere::setTopography(float* value)
{   
    std::vector<float> difference = std::vector<float>();
    std::transform(value, value +_worldDimension.getArea(),hmap.raw_data(),std::back_inserter(difference),[](float a, float b){return a-b;});

        std::for_each(plates, plates+num_plates,[&](auto& pla)
        {
            const uint32_t i = std::distance(plates, &pla);
            ...
            const float* tmpMap;
            float*  this_map;
            pla->getMap(&tmpMap,nullptr);
            this_map = (float*)tmpMap;
            // Copy first part of plate onto world map.
            if(x1 > _worldDimension.getWidth() && y1 >  _worldDimension.getHeight())
            {
                for (uint32_t y = y0; y < y1; ++y)
                {
                    const uint32_t y_world =  _worldDimension.yMod(y);
                    for (uint32_t x = x0; x < x1; ++x)
                    {
                        const uint32_t world_coordinate = _worldDimension.indexOf(_worldDimension.xMod(x),y_world);
                        if((imap[world_coordinate] == i))
                        {
                          *this_map += difference[world_coordinate]; //should not be 0.0. guaranteed by rock/cylce.fs
                        }
            ...
            else if(x1 > _worldDimension.getWidth())
            {
                for (uint32_t y = y0; y < y1; ++y)
                {
                    for (uint32_t x = x0; x < x1; ++x)
                    {
                        const uint32_t world_coordinate = _worldDimension.indexOf(_worldDimension.xMod(x), y);
                        if((imap[world_coordinate] == i))
                        {
                          *this_map += difference[world_coordinate]; 
                        }
            ...
            else if(y1 >  _worldDimension.getHeight())
            {
                for (uint32_t y = y0; y < y1; ++y)
                {
                    const uint32_t y_world =  _worldDimension.yMod(y);
                    for (uint32_t x = x0; x < x1; ++x)
                    {
                        const uint32_t world_coordinate = _worldDimension.indexOf(x,  y_world);
                        if((imap[world_coordinate] == i))
                        {
                          *this_map += difference[world_coordinate];
                        }
            ...
            else
            {
                 for (uint32_t y = y0; y < y1; ++y)
                {
                    for (uint32_t x = x0; x < x1; ++x)
                    {
                        const uint32_t world_coordinate = _worldDimension.indexOf(x, y);
                        if((imap[world_coordinate] == i))
                        {
                          *this_map += difference[world_coordinate];
                        }
        ...
        uint32_t& continental_collisions)
{
    ...
    std::for_each(plates,plates + num_plates,[&](auto& pla)
    {
        uint32_t i = std::distance(plates, &pla);
        ...
        uint32_t x_mod_start = (x0>= world_width) ? x0- world_width : x0;
        uint32_t y_mod = (y0>= world_height) ? y0- world_height : y0;
                ...
                y_mod = ++y_mod >= world_height ? y_mod - world_height : y_mod)
        {
                    ...
                    x_mod = ++x_mod >= world_width ? x_mod - world_width : x_mod)
            {
                ...
                float* p_hmap = &hmap[k];
                const float val_this_map =  this_map[j];
                uint32_t* p_imap = &imap[k];
                
                if (val_this_map < 2 * FLT_EPSILON) // No crust here...
                    continue;
                
                if (*p_imap >= num_plates) // No one here yet?
                {
                ...
                const bool prev_is_oceanic = *p_hmap < CONTINENTAL_BASE;
                const bool this_is_oceanic = val_this_map < CONTINENTAL_BASE;

                const uint32_t prev_timestamp = plates[*p_imap]->
                                                getCrustTimestamp(x_mod, y_mod);
                const uint32_t this_timestamp = this_age[j];
                const uint32_t prev_is_bouyant = (*p_hmap >val_this_map) |
                                                 ((*p_hmap + 2 * FLT_EPSILON > val_this_map) &
                                                  (*p_hmap < 2 * FLT_EPSILON +val_this_map) &
                ...
                if (this_is_oceanic & prev_is_bouyant) {
                    ...
                    pla->setCrust(x_mod, y_mod, val_this_map -
                    ...
                    const float sediment = SUBDUCT_RATIO * OCEANIC_BASE *
                                           (CONTINENTAL_BASE - *p_hmap) /
                                           CONTINENTAL_BASE;

                    plateCollision coll(*p_imap, x_mod, y_mod, sediment);
                    subductions[i].push_back(coll);
                    ++oceanic_collisions;

                    plates[*p_imap]->setCrust(x_mod, y_mod, *p_hmap -
                ...
                this->resolveJuxtapositions(i, j, k, x_mod, y_mod,
...
//        }
        ...
        std::for_each(plates,plates+ num_plates,[](auto& plate){plate->resetSegments(); plate->move();} );
        ...
        std::for_each(plates, plates+num_plates,[&](auto& pla)
        {
            ...
            for (uint32_t y = y0, j = 0; y < y1; ++y)
            {
                ...
                for (uint32_t x = x0; x < x1; ++x, ++j)
                {
                    ...
                    const uint32_t world_coord = _worldDimension.indexOf(x_mod, y_mod);
                    const float h0 = hmap[world_coord];
                    const float h1 = this_map[j];
                    const uint32_t a0 = amap[world_coord];
                    const uint32_t a1 =  this_age[j];

                    amap[world_coord] = (h0 *a0 +h1 *a1) /(h0 +h1);
