...
void plate::setCrust(uint32_t x, uint32_t y, float z, uint32_t t)
{
    ...
    float* map_point = &map[index];
    const uint32_t old_crust = -(*map_point > 0);
    const uint32_t new_crust = -(z > 0);
    t = (t & ~old_crust) | ((uint32_t)((*map_point * age_map[index] + z * t) /
                                       (*map_point + z)) & old_crust);
    age_map[index] = (t & new_crust) | (age_map[index] & ~new_crust);

    _mass.incMass(-1.0f * (*map_point));
    _mass.incMass(z);      // Update mass counter.
    *map_point = z;     // Set new crust height to desired location.
}
